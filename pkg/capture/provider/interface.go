...
type NetworkCaptureProviderInterface interface {
	...
	Setup(captureJobName, nodeHostname, requestTime string) (string, error)
	// CaptureNetworkPacket capture network traffic per user input and store the captured network packets in local directory.
	CaptureNetworkPacket(filter string, duration, maxSize int, sigChan <-chan os.Signal, requestTime string) error
